    ...
    try:
        ...
        for email in assigned:
            if email not in existing:
                ...
                last = ""
                try:
                    raw_user = redis_client.get(f"user:{email}")
                    if raw_user:
                        udata = json.loads(raw_user)
                ...
                if not first or not last:
                    try:
                        raw_student = redis_client.get(f"student:{email}")
                        if raw_student:
                            sdata = json.loads(raw_student)
                            first = first or sdata.get("first_name", "")
                            last = last or sdata.get("last_name", "")
                    except Exception:
                        pass
